...
public class JazzyViewPager extends ViewPager {
    ...
    // 是否可以滑动翻页，因为有些需要翻页，有些不需要，所以设计成可以设置是否翻页
    private boolean                  mEnabled        = true;
    
    // 为了解决ViewPager嵌套时的问题，只要设置了该参数（父控件ViewPager），就会让当前ViewPager抢占父控件ViewPager的事件
    ...
    private static final float       ZOOM_MAX        = 0.85f;
    
    private static final float       ROT_MAX         = 15.0f;
    
    private static final float       ALPHA_MAX       = 0.3F;
    ...
    private void animateZoom(View left, View right, float positionOffset, boolean in) {
        ...
        if (mState != State.IDLE) {
            if (left != null) {
                ...
                mAlpha = in ? ALPHA_MAX + (1 - ALPHA_MAX) * (1 - positionOffset) : 1 + ALPHA_MAX - ALPHA_MAX * (1 - positionOffset);
                ViewHelper.setPivotX(left, left.getMeasuredWidth() * 0.5f);
                ViewHelper.setPivotY(left, left.getMeasuredHeight() * 0.5f);
                ViewHelper.setScaleX(left, mScale);
                ViewHelper.setScaleY(left, mScale);
                ViewHelper.setAlpha(left, mAlpha);
            }
            if (right != null) {
                manageLayer(right, true);
                mScale = in ? ZOOM_MAX + (1 - ZOOM_MAX) * positionOffset : 1 + ZOOM_MAX - ZOOM_MAX * positionOffset;
                mAlpha = in ? ALPHA_MAX + (1 - ALPHA_MAX) * positionOffset : 1 + ALPHA_MAX - ALPHA_MAX * positionOffset;
                ViewHelper.setPivotX(right, right.getMeasuredWidth() * 0.5f);
                ViewHelper.setPivotY(right, right.getMeasuredHeight() * 0.5f);
    ...
    public void cleanAnim() {
        ...
        View v;
        for (int i = 0; i < getChildCount(); i++) {
            Object o = mObjs.get(Integer.valueOf(i));
            if (o == null) {
                continue;
            }
            v = getChildAt(i);
